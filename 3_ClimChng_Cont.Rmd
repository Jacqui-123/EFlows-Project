...
Variables Calculated: IHA, ice cycle, and percent change
...
# Tidying Pre climate change stns, 1975-2005
```{r}
...
# Tidying- Post climate change stns 2006-2023
```{r}
...
#Calculate percent change, ice variables
```{r}
df_lst <- list()
...
DY <- calc_MK(df_ice_post_final, parameter = Day_of_year, start = 2007) %>%
  rename("Result_Freshet_Onset" = 'P_Value' )

icecover <- calc_MK(df_ice_post_final, parameter = ice_coverage_wy, start = 2007)%>%
  rename("Result_ice_cover" = 'P_Value' )

frdoy <- calc_MK(df_ice_post_final, parameter = Freeze_DOY, start = 2007) %>%
  rename("Result_freeze_doy" = 'P_Value' )

thawdoy <- calc_MK(df_ice_post_final, parameter = Thaw_DOY, start = 2007)%>%
  rename("Result_thaw_doy" = 'P_Value' )

result <- cbind(DY, icecover, frdoy, thawdoy)
MK_result_final <- result[!duplicated(as.list(result))] #removed duplicated columns, may have to delete because it will remove interpretation columns if they're exactly the same
...
Reversals <- calc_MK(IHA_post_expand, parameter = Reversals, start = 2007) %>%
  rename("Result_Reversals" = 'P_Value')

One_Day_Min <- calc_MK(IHA_post_expand, parameter = One_Day_Min , start = 2007) %>% rename("Result_One_Day_Min" = 'P_Value')

One_Day_Max <-  calc_MK(IHA_post_expand, parameter = One_Day_Max , start = 2007) %>% rename("Result_1_Day_Max" = 'P_Value')

Seven_Day_Min <- calc_MK(IHA_post_expand, parameter = Seven_Day_Min , start = 2007) %>% rename("Result_7_Day_Min" = 'P_Value')

Seven_Day_Max <-  calc_MK(IHA_post_expand, parameter = Seven_Day_Max , start = 2007) %>% rename("Result_7_Day_Max" = 'P_Value')

Thirty_Day_Min <- calc_MK(IHA_post_expand, parameter = Thirty_Day_Min , start = 2007) %>% rename("Result_30_Day_Min" = 'P_Value')

Thirty_Day_Max <-  calc_MK(IHA_post_expand, parameter = Thirty_Day_Max , start = 2007) %>% rename("Result_30_Day_Max" = 'P_Value')

High_pulse_number <-  calc_MK(IHA_post_expand, parameter = High_pulse_number , start = 2007) %>% rename("Result_High_pulse_number" = 'P_Value')

Low_pulse_number <-  calc_MK(IHA_post_expand, parameter = Low_pulse_number , start = 2007) %>% rename("Result_Low_pulse_number" = 'P_Value')

High_pulse_length <-  calc_MK(IHA_post_expand, parameter = High_pulse_length , start = 2007) %>% rename("Result_High_pulse_length" = 'P_Value')

Low_pulse_length <-  calc_MK(IHA_post_expand, parameter = Low_pulse_length , start = 2007) %>% rename("Result_Low_pulse_length" = 'P_Value')
...
#write.csv(result_MK_IHA, "IHAresult3.csv")
