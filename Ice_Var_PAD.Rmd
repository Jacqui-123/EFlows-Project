...
for (i in unique(stn_07BK007_wy_doy$waterYear)) { #first loop
  #subset data by year, resetting at each year
  index <- 0 
  f_index <- 16 
  df_subset <- stn_07BK007_wy_doy[stn_07BK007_wy_doy$waterYear == i,]

  #calc rolling 16 day mean
  rollmn <- rollmean(df_subset$Value, k = 16, width = 16)
  
    for (i in rollmn) { #second loop
      #increment index
      index <- index + 1
      f_index <- f_index + 1
      ...
      flowvalue <- df_subset$Value[f_index] #flow = 0.654
        ...
        if (flowvalue > rollmnvalue1.5) { #third loop
        #append date, flow value to a list here
        ...
        print(fl)
        print("end")
        Freshet_Date <- as.Date(unlist(date))
        Freshet_Flow <- unlist(flow)
        df <- cbind.data.frame(Freshet_Date, Freshet_Flow)
        return(df)
        break
        }
...
# the problem now is that it only saves the first value, and exits the loop
