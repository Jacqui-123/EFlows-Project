...
Workflow for the calculations and graphing for the freeze-up dates, ice break-up dates, and continuous ice coverage for a couple of stations on the PAD. 

Note: Timing of Spring freshet initiation (1.5 x 16-day running mean) calculations are currently in progress. See github: https://github.com/Jacqui-123/EFlows-Project 
...
#add day of year
stn_wy_doy <- day_of_year_calc(stn_wy)
...
#test
#Ice_coverage_stn07bk007 <- Group_1_ice_cover(stn_07BK007_wy_doy)
...
#test
#Fr_thaw_stn07bk007 <- Group_2_freeze_thaw(stn_07BK007_wy_doy)
#edit function so that it takes water year and calendar year arguments
...
# calculation for a running mean

#find the index of March 1... 

start_index <- which(stn_07BK007_1972$Date == "1972-03-01")

start_date <- as.Date(paste0("-", "03-01"), format = "%Y-%m-%d")

years <- unique(format(stn_07BK007_1972$Date, "%Y"))

start_index <- which(stn_07BK007_1972$Date == start_date)

rollmean <- rollmean(stn_07BK007_1972$Value[84:99], k = 16)


for (i in years) { 

if (stn_07BK007_1972$Value[94] > 1.5*rollmean){ 
...
From March 1-16, March 17- April 2, etc until end of June, take 16 day running mean. As soon as the daily flow is > 1.5 that mean, that is the onset of freshet. 

#running mean - from doy == 60, calculate 16 day running mean
#OR just get rid of anything where doy <= 60

stn_07BK007_wy_doy
  rollmean(df$Value, k = 16)
  ?rollmean
  

temp <- stn_07BK007_wy_doy %>%
  filter(waterYear == '1972') %>%
  filter(day_of_year > 60) %>%
  na.omit()

#do this... then when it stops being true print the index?
x <- rollmean(temp$Value, k = 16) 

x[3]

  
library(data.table)
data.table::frollmean(temp$Value, 16)


####
window <- 

 for (i in unique(temp$waterYear)) {
   
   
   mn <- mean(temp$Value[1:16])
   
   if mn >= (temp$Value[17]) *1.5
