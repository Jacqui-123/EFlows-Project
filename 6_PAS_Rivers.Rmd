...
#### For the first time, scientists can analyze a river’s ice cycle using both historical and real-time data from the Water Survey of Canada database. Newly developed functions are available and applicable at scale on any river in Canada with seasonal ice coverage. They grant scientists and watershed managers the tools to conduct reproducible research at scale, and enable data-driven decisions based on real-time data.
...
The Canadian River Ice Database Project previously calculated some of these ice cycle variables, however, they are for a select group of rivers, and were only calculated through 2015. These newly developed functions allow scientists to calculate real-time, present-day data, in addition to historical data, for any water station from the Water Survey of Canada.  

The next step in this project will be to create an accessible, open-source R package for these functions. My hope is that scientists and watershed managers will be able to use this tool in conjunction with climatic data and models, to better assess and predict broad-scale watershed changes. 
