...
#et gross discharge for all stations and join to stn_all
...
pltree(hclust2, cex = 0.6, hang = -1, main = "Regime shape: Full year data")

#agglomerative coefficient is low, 0.522
#hclust2$ac 
```
...
#none look that strong, likely no clusters?
```
...
#cluster with ward linkage method, ac = .85
hclust3 <- agnes(stns_seas_z, method = "ward") 
pltree(hclust3, cex = 0.6, hang = -1, main = "Regime shape: seasonal data")


#plot cluster with groups = 3
sub_grp <- cutree(hclust3, k = 3)
pltree(hclust3, cex = 0.6)
rect.hclust(hclust3, k = 3, border = 2:5)

cutree(as.hclust(hclust3), k = 3)

fviz_nbclust(stns_seas_z, FUN = hcut, method = "silhouette")

#could be two groups or 3 groups 
#when sure can add to data: 
...
library(leaflet)
pal <- colorFactor(palette = c("red", 'blue', "green"), domain = stns_seas_clusters$cluster_number)

leaflet(seas_clust_latlong) %>% addTiles() %>%
  addCircleMarkers(lng = ~LONGITUDE, lat = ~LATITUDE, color = ~pal(cluster_number), fillColor = ~pal(cluster_number), fillOpacity = 0.6, radius = 5, stroke = FALSE)
...
#calc variables by water year, and then get medians for whole time period
 stns_regsize_full <- stns_daymonthyear_full %>%
  group_by(STATION_NUMBER, waterYear) %>%
  summarise(median_runoff = median(runoff),
         stdev_runoff = sd(runoff),
         max_runoff = max(runoff),
         min_runoff = min(runoff)) %>%
  group_by(STATION_NUMBER) %>%
  summarise(median_median_runoff = median(median_runoff),
...
hclustd <- agnes(stns_regsize_full_z, method = "complete") 
pltree(hclustd, cex = 0.6, hang = -1, main = "Regime size: Full year data")
...
hclustd2 <- agnes(stns_regsize_seas_z, method = "ward") 
pltree(hclustd2, cex = 0.6, hang = -1, main = "Regime size: Full year data")
...
#plot cluster with groups = 3
sub_grp <- cutree(hclustd2, k = 4)
pltree(hclustd2, cex = 0.6)
rect.hclust(hclustd2, k = 4, border = 2:5)

cutree(as.hclust(hclustd2), k = 4)
